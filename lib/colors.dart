import 'package:flutter/material.dart';

class Colors {
  static Color backgroundColor = const Color(0xFFF3F4F5);
  static Color accentColor = const Color(0xFF0666EB);
  static Color disable = const Color(0xFF8B94A0);
  static Color orangeColor = const Color(0xFFFD8509);
  static Color greenColor = const Color(0xFF1DC14F);
  static Color grayColor = const Color(0xFF8B94A0);
  static Color recordColor = const Color(0xFFE59FA2);
  static Color iconBackgrounfColor = const Color(0xFFEDF5FD);
  static Color secondaryTextColor = const Color(0xFF717276);
}
